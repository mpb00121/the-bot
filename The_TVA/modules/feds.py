...
import The_Tva.modules.sql.feds_sql as sql
from The_Tva import DRAGONS, EVENT_LOGS, LOGGER, OWNER_ID, TIGERS, WOLVES, dispatcher
from The_Tva.modules.disable import DisableAbleCommandHandler
from The_Tva.modules.helper_funcs.alternate import send_message
from The_Tva.modules.helper_funcs.chat_status import is_user_admin
from The_Tva.modules.helper_funcs.extraction import (
    extract_unt_fedban,
    extract_user,
    extract_user_fban,
)
from The_Tva.modules.helper_funcs.string_handling import markdown_parser
...
def new_fed(update: Update, context: CallbackContext):
    ...
    if not fednam == "":
        ...
        if not x:
            update.effective_message.reply_text(
                "Can't federate! Please contact @The_TVA  if the problem persist."
            )
            return
...
def join_fed(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                "Failed to join federation! Please contact @The_TVA should this problem persist!"
            )
            return
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if fban:
        fed_name = info["fname"]
        # https://t.me/The_TVA/41606 // https://t.me/The_TVA/41619
        ...
        if not x:
            message.reply_text(
                "Failed to ban from the federation! If this problem continues, contact @The_TVA."
            )
            return
...
def set_frules(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            update.effective_message.reply_text(
                "Whoa! There was an error while setting federation rules! If you wondered why please ask it in @The_TVA !"
            )
            return
